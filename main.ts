...
class Board {
    ...
    static tiles: string[][];
    static winStates: string[];
    static hoverTile = { x: null, y: null, bx: null, by: null };
    static playerTurn: string = 'X';
    static playableArea: number = 10; // 10=all cells, 0=only first cell, 1=only second cell, etc.
    ...
    static click(event: MouseEvent, xx: number, yy: number): void {
        var type = clickType(event);
        if ((type === 'right' || type === 'left') && Board.hoverTile.x !== null && Board.hoverTile.y !== null) {
            var bxy = Board.hoverTile.bx + Board.hoverTile.by * 3,
                xy = Board.hoverTile.x + Board.hoverTile.y * 3;
            ...
            if (bxy === Board.playableArea || Board.playableArea === 10) {
                ...
                if (Board.winStates[Board.playableArea] && Board.winStates[Board.playableArea] !== '') Board.playableArea = 10;
                ...
                if (Game.mode === 'AI') {
                    ...
                    if (Board.winStates[Board.playableArea] !== '') Board.playableArea = 10;
                } else {
    ...
    static checkForBoardWinners(): void {
    ...
    static checkForGameWinners(): void {
    ...
    static getBoardWinState(tiles: Array<string>): string {
    ...
    static render(): void {
        ...
        // playable area
        if (Board.playableArea === -1); // Game is over, don't render any playable area
        ...
        if (Board.hoverTile.x !== null && Board.hoverTile.y !== null) {
            ...
            Game.context.fillRect(Board.hoverTile.x * Board.tileSize + 10 + (Board.tileSize * 3 + 10) * Board.hoverTile.bx,
                Board.hoverTile.y * Board.tileSize + 10 + (Board.tileSize * 3 + 10) * Board.hoverTile.by, Board.tileSize, Board.tileSize);
            Game.context.fillStyle = 'white';
            Game.context.fillText(Board.playerTurn, Board.hoverTile.x * Board.tileSize + 10 + (Board.tileSize * 3 + 10) * Board.hoverTile.bx + (Board.playerTurn === 'X' ? 15 : 9),
                Board.hoverTile.y * Board.tileSize + 10 + (Board.tileSize * 3 + 10) * Board.hoverTile.by + 49);
        }
    }

    /* x, y: starting corner of grid, x+3w, y+3h: ending corner of grid */
    static drawGrid(x: number, y: number, colw: number): void {
    ...
    static setTile(board: number, position: number, player: string): boolean {
    ...
    static clear(mode: string): void {
        Board.winStates = new Array<string>(9);
        Board.tiles = new Array<Array<string>>(9);
        for (var i = 0; i < Board.tiles.length; i++) {
            Board.tiles[i] = new Array<string>(9);
...
class AI {
    ...
    static makeMove(playableArea: number) {
        window.setTimeout(AI.move, 500, playableArea);
    }

    static move(playableArea: number) {
        if (Board.isFull()) Game.over = true;
        if (Game.over) return;

        var b: number, p: number;

        if (0) {
        } else { // TODO LATER add better AI
            do {
        ...
        if (Board.winStates[p]) Board.playableArea = 10;
        else Board.playableArea = p;
        Game.render();
    }
}

function createPopup(type: string): void {
    switch (type) {
        case 'newgame':
...
function showPopup(html: string, css: string = ''): void {
